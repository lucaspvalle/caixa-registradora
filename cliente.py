...
class Caixa():
    ...
    def entrada (self):
        ...
        while comando_entrada == "S":
            ...
            cursor.execute ("SELECT produto,qtdade FROM estoque where produto = ?", (input_material,))
            conteudo = cursor.fetchone()

            if conteudo is not None:
                novo_estoque = conteudo [1] + input_qtdade
                cursor.execute ("UPDATE estoque SET qtdade = ? WHERE produto = ?", (novo_estoque, input_material))
                cnx.commit()

            else:
                input_codigo = input ("Insira um código para este material: ")
                input_valor = input ("Insira o valor de venda deste material: R$")
                cursor.execute ("INSERT INTO estoque VALUES (?, ?, ?, ?)", (input_material, input_qtdade, input_valor, input_codigo))
                cnx.commit()
    ...
    def atualizarProduto (self):
        ...
        while comando_atualizar == "S":
            ...
            cursor.execute("SELECT produto FROM estoque WHERE produto = ?", (produto_atualizado,))
            conteudo = cursor.fetchone()

            if conteudo is not None:
                atualizar_venda = input ("Digite o valor de venda a ser atualizado: R$")
                cursor.execute ("UPDATE estoque SET valor = ? WHERE produto = ?", (atualizar_venda, produto_atualizado))
                cnx.commit()
    ...
    def vendaMateriais (self, output_codigo):
        cursor.execute ("SELECT * FROM estoque WHERE codigo = ?", (output_codigo,))
        conteudo = cursor.fetchone()
        ...
        if conteudo is not None:
            ...
            if conteudo [2] >= peso:
                ...
                novo_estoque = conteudo [2] - peso
                cursor.execute ("UPDATE estoque SET qtdade = ? WHERE codigo = ?", (novo_estoque, output_codigo))
                cnx.commit()
    ...
    def resumoItens (self):
        cursor.execute ("SELECT produto,qtdade,valor FROM estoque")

        print ("\nRelatório Gerencial")
        print ("Produtos cadastrados no sistema")
        print ("\nProduto \t Qtdade (kg) \t Valor (R$)")

        for produto, qtdade, valor in cursor.fetchall():
            print (f"{produto} \t {qtdade:.2f} \t {valor}")
            
    def resumoBaixoEst (self):
        cursor.execute ("SELECT produto,qtdade FROM estoque WHERE qtdade < 5")
